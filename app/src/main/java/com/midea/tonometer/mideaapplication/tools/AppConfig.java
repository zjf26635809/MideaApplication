...
public class AppConfig {
	...
	public AppConfig() {
		Context delegate = MWApplication.getContext();
	...
	public static String getDisplayString() {
		...
		dm = MWApplication.getContext().getResources().getDisplayMetrics();
	...
	public static String getUUID() {
		final TelephonyManager tm = (TelephonyManager) MWApplication.getContext()
				.getSystemService(Context.TELEPHONY_SERVICE);
		final String tmDevice, tmSerial, androidId;
		tmDevice = "" + tm.getDeviceId();
		tmSerial = "" + tm.getSimSerialNumber();
		androidId = ""
				+ android.provider.Settings.Secure.getString(
				MWApplication.getContext().getContentResolver(),
				android.provider.Settings.Secure.ANDROID_ID);
		UUID deviceUuid = new UUID(androidId.hashCode(),
				((long) tmDevice.hashCode() << 32) | tmSerial.hashCode());
		String uniqueId = deviceUuid.toString();
	...
	public static int getStatusHeight() {
		...
		try {
			...
			int height = Integer.parseInt(clazz.getField("status_bar_height")
					.get(object).toString());
			statusHeight = MWApplication.getContext().getResources().getDimensionPixelSize(height);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return statusHeight;
